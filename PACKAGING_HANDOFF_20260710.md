...
- 循环等级仅读取底部八个数值：极乐净土、泰特拉、米西利斯、朝圣者、反常、火力型、防御型、辅助型。支持 `1-999`；`251-999` 必须带完整 `Lv` 前缀且高置信度才接受。尾部形态修复仅处理至少连续两位的 `6`，例如 `66 -> 99`、`166 -> 199`、`266 -> 299`、`366 -> 399`、`666 -> 999`；真实的 `116`、`66`、`96` 等值保持原样。若全局检测出现多个 `Lv` 数字合框或异常文本，则只对相关槽位追加批量数字行确认，三次以上一致且不短于原值才覆盖。
